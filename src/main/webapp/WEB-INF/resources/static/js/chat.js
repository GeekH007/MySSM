 ...
 */
function getConnect() {
    //var path = window.location.hostname + ":8080/" + window.location.pathname.split("/")[1];
    var path = window.location.hostname + ":8080/";
    if (window.WebSocket) {
        console.log('Support WebSocket.');
        //websocket = new WebSocket('ws://' + path + '/chatHandler');
        websocket = new WebSocket('ws://' + path + '/websocket.html');
    } else {
        ...
        websocket = new SockJS('http://' + path + '/sockjs/websocket.html')
    }
...
function handleChatMessage(httpSessionId, username, payload) {
    if (localSessionId === httpSessionId) {
        $chatArea.append(
            ...
            '<img src="/resources/static/image/chat-ico.png">' +
            '</div>' +
            '</div>'
        );
    } else {
